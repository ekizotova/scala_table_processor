...
class HeadersHandler(var next: Option[CliHandler]) extends CliHandler {
  ...
  override def handle(args: Array[String], options: scala.collection.mutable.Map[String, Any],
    ...
    if (args.nonEmpty && args.head == "--headers") {
  ...
  override def describe(): Option[String] = Some(
    "Output: --headers : Turns on printing of headers (optional, by default, they are not printed)."
  )
}

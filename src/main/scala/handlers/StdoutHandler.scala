...
class StdoutHandler(var next: Option[CliHandler]) extends CliHandler {
  ...
  override def handle(args: Array[String], options: scala.collection.mutable.Map[String, Any], filters: scala.collection.mutable.Buffer[TableFilter]): Array[String] = {
    if (args.nonEmpty && args.head == "--stdout") {
      options("stdout") = true
      args.tail
    } else {
  ...
  override def describe(): Option[String] = Some(
    "Output: --stdout : Print the table to the standard output (optional, by default true)."
  )
}
